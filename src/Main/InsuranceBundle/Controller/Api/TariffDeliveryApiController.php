...
namespace Main\InsuranceBundle\Controller\Api;
...
class TariffDeliveryApiController extends BaseController
{
...
*/
    ...
    /**
     * @Method({"GET"})
     * @Route("/manager/survey/{type}/result", name="manager_survey_type_result")
     */
    ...
    )
    {
        ...
        $urlBuilder->setShowMissing(false);
        $urlBuilder->build();
        $url = $urlBuilder->get();
        ...
        */
        if ($structure) {
            if (!empty($data) && isset($data['daten'])) {
                ...
                */
                ...
                if (count($this->cheapestList) > 0
                    && count($this->cheapestList) > 0
                    && count($this->cheapestList) > 0
                ) {
        ...
        $tmpCurrentContracts = $contractRepository->findBy(
            [
                'user' => $this->user,
                'contractType' => $this->type
            ]
        );
        if (null != $tmpCurrentContracts) {
            foreach ($tmpCurrentContracts as $currentContract) {

                $paymentInterval = $currentContract->getPaymentInterval();
                $fee = (null != $currentContract->getPaymentFee()) ? $currentContract->getPaymentFee() : 0;
                if (12 == $paymentInterval) {
                    $fee = $currentContract->getPaymentFee() * 12;
                } else if (6 == $paymentInterval) {
                    $fee = $currentContract->getPaymentFee() * 2;
                } else if (3 == $paymentInterval) {
                    $fee = $currentContract->getPaymentFee() * 4;
                }

                $currentContracts[] = [
                    'exist' => 1,
                    'logo' => $currentContract->getCompany()->getLogo(),
                    'tariffName' => $currentContract->getTariffName(),
                    'fee' => $fee
                ];
            }
        }
        return $this->render('@MainUserBundle/Manager/survey.api.result.html.twig', [
                'user' => $this->user,
                'type' => $typeIdentifier,
     ...
     */
        ...
        foreach ($this->cheapestList as $aKey => $aValue) {
            if (isset($aKey) && isset($aValue) && isset($aValue[0]) && isset($aValue[1])) {
                ...
                echo "CT: " . $aValue[0] . "<br>";
                print_r($cheapestTariff[0]['fee']);
                print_r($cheapestTariff);
                echo "<br><br>";
                */
            }
        }
        ...
        foreach ($this->bestFitList as $aKey => $aValue) {
            ...
            */
                $first = str_pad(($aValue[0]), 10, '0', STR_PAD_LEFT);
                $second = str_pad(str_replace(",", "", $aValue[1][0]), 10, '0', STR_PAD_LEFT);
                $tmpKeyAs = floatval($first) / floatval($second);

                $keyAs = str_pad(str_replace(",", "", $tmpKeyAs), 10, '0', STR_PAD_LEFT);
                $temp[$keyAs] = [$aKey, [$aValue[0], [$aValue[1][0], $aValue[1][1]]]];
                //$temp[$first . "oldkey" . $second] = [$aKey, [$aValue[0], [$aValue[1][0], $aValue[1][1]]]];
            }
        }
        //ksort($temp);
    ...
    private function getTurn($entryName)
    {
