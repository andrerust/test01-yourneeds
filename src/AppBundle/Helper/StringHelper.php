...
class StringHelper
{
    ...
    public static function cutMiddlePart(String $string, int $maxLength)
    {
        $textLength = ($maxLength > 1) ? $maxLength : strlen($string);
        $maxChars = 70;
        $result = substr_replace($string, '...', $maxChars / 2, $textLength - $maxChars);
        return $result;
    }
    
    public static function convertStringToDate(String $string)
    {
        if ($string) {
